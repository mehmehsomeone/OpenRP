...
qboolean trap_R_inPVS( const vec3_t p1, const vec3_t p2, byte *mask ) {
	return (qboolean)syscall( CG_R_INPVS, p1, p2, mask );
}
...
{
	return (qboolean)syscall(CG_G2_SETSKIN, ghoul2, modelIndex, customSkin, renderSkin);
}
...
{
	return (qboolean)syscall(CG_G2_GETBONEANIM, ghoul2, boneName, currentTime, currentFrame, startFrame, endFrame, flags, animSpeed, modelList, modelIndex);
}
...
{
	return (qboolean)syscall(CG_G2_GETBONEFRAME, ghoul2, boneName, currentTime, currentFrame, modelList, modelIndex);
}
...
{
	return (qboolean)syscall(CG_G2_SKINLESSMODEL, ghlInfo, modelIndex);
}
...
{
	return (qboolean)syscall(CG_G2_ATTACHENT, boltInfo, ghlInfoTo, toBoltIndex, entNum, toModelNum);
}
...
{
	return (qboolean)syscall(CG_G2_DOESBONEEXIST, ghoul2, modelIndex, boneName);
}
...
{
	return (qboolean)syscall(CG_G2_RAGPCJCONSTRAINT, ghoul2, boneName, min, max);
}
...
{
	return (qboolean)syscall(CG_G2_RAGPCJGRADIENTSPEED, ghoul2, boneName, PASSFLOAT(speed));
}
...
{
	return (qboolean)syscall(CG_G2_RAGEFFECTORGOAL, ghoul2, boneName, pos);
}
...
{
	return (qboolean)syscall(CG_G2_GETRAGBONEPOS, ghoul2, boneName, pos, entAngles, entPos, entScale);
}

qboolean trap_G2API_RagEffectorKick(void *ghoul2, const char *boneName, vec3_t velocity) //add velocity to a rag bone
{
	return (qboolean)syscall(CG_G2_RAGEFFECTORKICK, ghoul2, boneName, velocity);
}
...
{
	return (qboolean)syscall(CG_G2_RAGFORCESOLVE, ghoul2, force);
}
...
{
	return (qboolean)syscall(CG_G2_SETBONEIKSTATE, ghoul2, time, boneName, ikState, params);
}

qboolean trap_G2API_IKMove(void *ghoul2, int time, sharedIKMoveParams_t *params)
{
	return (qboolean)syscall(CG_G2_IKMOVE, ghoul2, time, params);
}

qboolean trap_G2API_RemoveBone(void *ghoul2, const char *boneName, int modelIndex)
{
	return (qboolean)syscall(CG_G2_REMOVEBONE, ghoul2, boneName, modelIndex);
}
...
{
	return (qboolean)syscall(CG_G2_OVERRIDESERVER, serverInstance);
}
