...
*/
	...
	{	// Make sure we don't do the thirdperson model effects for the local player if we're in first person
		...
		if(cg.predictedPlayerState.weapon == WP_BOWCASTER && (cent->currentState.eFlags & EF_ALT_FIRING)) return;
...
qboolean CG_WeaponCheck(int weap)
{
	...
	*/
	...
	if(weap == WP_GRENADE && cg.predictedPlayerState.ammo[weaponData[weap].ammoIndex] < 1)
		return qfalse;
...
static qboolean CG_WeaponSelectable( int i ) {
	...
	//[Reload]
	if(i == WP_GRENADE && cg.predictedPlayerState.ammo[weaponData[i].ammoIndex] < 1)
		return qfalse;
...
void CG_DrawWeaponSelect( void ) {
	...
	if ( !CG_WeaponSelectable(cg.weaponSelect) &&
		(cg.weaponSelect == WP_THERMAL || cg.weaponSelect == WP_GRENADE) )
...
=================
*/
extern void FX_TuskenHitWall(vec3_t origin, vec3_t normal);
extern void FX_TuskenWeaponHitPlayer( vec3_t origin, vec3_t normal);
...
//	CG_Bleed( origin, entityNum );
	...
	case WP_TURRET:
		...
		break;
	case WP_TUSKEN_RIFLE:
		FX_TuskenWeaponHitPlayer(origin,dir);
		break;
