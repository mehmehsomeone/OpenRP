...
#ifdef QAGAME
		{
			//pVeh->m_pVehicleInfo->maxSlope = 0.7;
			pVeh->m_pVehicleInfo->hoverStrength = 100;
			pVeh->m_pVehicleInfo->hoverHeight = 50;
			pitch_roll_for_slope(&g_entities[pVeh->m_pParentEntity->playerState->clientNum],vec3_origin);
			//g_entities[pVeh->m_pParentEntity->playerState->clientNum].client->ps.iModelScale = 50;
		}
#endif
	}
...
void AnimateRiders( Vehicle_t *pVeh )
{
	...
	else 
	{
		...
		bool		Turbo		= (qboolean)(curTime<pVeh->m_iTurboTime);
