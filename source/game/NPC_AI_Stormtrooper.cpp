...
void ST_FleeFromGrenade( gentity_t *self, gentity_t *enemy, vec3_t dangerPoint)
{
	trace_t trace;
	vec3_t	fallForwardDir, fallRightDir, end;
	...
	VectorCopy(vec3_origin,end);
	end[1] = self->client->ps.viewangles[1];
	AngleVectors( end, fallForwardDir, fallRightDir, NULL );
	VectorNormalize( fallForwardDir );
	VectorMA( self->r.currentOrigin, 245 + 10, fallForwardDir, end );

	//G_StartFlee( self, enemy, dangerPoint, dangerLevel, minTime, maxTime );
	trap_Trace(&trace,self->r.currentOrigin,self->r.mins,self->r.maxs,end,self->s.clientNum,MASK_SOLID);

	if(!trace.allsolid)
	{
		NPC_SetMoveGoal( self, end, 8, qfalse, -1, NULL );
	}
...
void ST_FallToDeath(gentity_t*ent)
{
	if(!ent || !ent->client)
	{
		return;
	...
	for ( i = curMemberNum; i < lastMemberNum; i++ )
	{
		...
		*/
		...
		{ 
			...
			TIMER_Set (NPC, "checkGrenadeTooCloseDebouncer", Q_irand(100, 200));
				...
				if ( ent->s.eType == ET_MISSILE )
				{
					...
					{//a thermal
						...
						{//bounced and an enemy thermal
							ST_Speech( NPC, SPEECH_COVER, 0 );//FIXME: flee sound?
							//ST_StartFlee(NPC,NPC->enemy, ent->r.currentOrigin, AEL_DANGER_GREAT, 1000, 2000);
							ST_FleeFromGrenade(NPC,NPC->enemy,ent->r.currentOrigin);
