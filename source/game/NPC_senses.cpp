...
{//remember this alert.  Also add this to NPC's danger sense for navigation purposes.
	...
	//--------------------------------------------------------------------------------------
	IsDangerous = (qboolean)(at->level >= AEL_DANGER);
	IsFromNPC	= (qboolean)(at->owner && at->owner->client);
	IsFromEnemy = (qboolean)(IsFromNPC && at->owner->client->playerTeam!=self->client->playerTeam);
