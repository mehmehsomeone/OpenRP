...
//[OpenRP - Compiler complained when these weren't extern'd]
extern int		(*ENG_FS_FOpenFileByMode)	(const char *qpath, fileHandle_t *f, fsMode_t mode);
extern void	(*ENG_FS_FCloseFile)		(fileHandle_t f);
extern int		(*ENG_FS_Write)				(const void *buffer, int len, fileHandle_t f);
extern char *	(*ENG_FS_BuildOSPath)		(const char *base, const char *game, const char *qpath);
extern char **	(*ENG_Sys_ListFiles)		(const char *directory, const char *extension, char *filter, int *numfiles, int wantsubs);
extern void	(*ENG_Sys_FreeFileList)		(char **list);
extern char *	(*ENG_Sys_Cwd)				(void);
extern cvar_t *(*ENG_Cvar_Get)				(const char *var_name, const char *var_value, int flags);
//[/OpenRP - Compiler complained when these weren't extern'd]
