...
//	char*	type = NULL;
	...
	{
		int n = atoi( arg2 );
		if((Q_stricmp(arg2, "0") && n == 0))    //That massive strchr check was just ugly, and didn't offer any protection either way.
		{
			trap_SendServerCommand( ent->s.number, "print \"Only numbers are allowed.\n\"" );
			return;
		}       
		else if( n < 0 || n >= 60 )             //TODO: make this a cvar
		{
			trap_SendServerCommand( ent->s.number, "print \"Invalid second argument for map_restart.\nValid values are 0-60\n\""); //TODO: insert cvar value
			return;
		}
		Com_sprintf( level.voteString, sizeof(level.voteString), "%s %s", arg1, arg2 ); //Your code for the number check had no effect.
		Com_sprintf( level.voteDisplayString, sizeof( level.voteDisplayString ), "%s", level.voteString ); //Was also missing the display string
	}
	//[/JKH Bugfix]
	else
	{
