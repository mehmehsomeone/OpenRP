...
int BotAISetupClient(int client, struct bot_settings_s *settings, qboolean restart) {
	...
	bs->botWeaponWeights[WP_TUSKEN_RIFLE] = 13;
		...
		{ //try to escape from anything with a non-0 s.weapon and non-0 damage. This hopefully only means dangerous projectiles.
		  //Or a sentry gun if bolt_Head == 1000. This is a terrible hack, yes.
			...
			if (ent->s.weapon != WP_THERMAL && ent->s.weapon != WP_FLECHETTE &&
				//[NewWeapons][EnhancedImpliment]
				//ent->s.weapon != WP_DET_PACK && ent->s.weapon != WP_GRENADE && ent->s.weapon != WP_GRENADE_2)
				ent->s.weapon != WP_DET_PACK && ent->s.weapon != WP_GRENADE)
				//[/NewWeapons][EnhancedImpliment]
			{
				factor = 0.5;
...
void StandardBotAI(bot_state_t *bs, float thinktime)
{
...
#endif
	{
		...
		{ //try light side powers
			...
			else if ((bs->cur_ps.fd.forcePowersKnown & (1 << FP_MINDTRICK)) && bs->frame_Enemy_Len < MAX_TRICK_DISTANCE && level.clients[bs->client].ps.fd.forcePower > forcePowerNeeded[level.clients[bs->client].ps.fd.forcePowerLevel[FP_MINDTRICK]][FP_MINDTRICK] && InFieldOfVision(bs->viewangles, 50, a_fo) && !(bs->currentEnemy->client->ps.fd.forcePowersActive & (1 << FP_SEE)))
			{
				level.clients[bs->client].ps.fd.forcePowerSelected = FP_MINDTRICK;
				useTheForce = 1;
				forceHostile = 1;
			}
			...
			else if ((bs->cur_ps.fd.forcePowersKnown & (1 << FP_MANIPULATE)) && g_entities[bs->client].health < 35 && level.clients[bs->client].ps.fd.forcePower > forcePowerNeeded[level.clients[bs->client].ps.fd.forcePowerLevel[FP_MANIPULATE]][FP_MANIPULATE])
			{
				level.clients[bs->client].ps.fd.forcePowerSelected = FP_MANIPULATE;
				useTheForce = 1;
				forceHostile = 0;
			}
		}
		...
		//[/SaberSys]
	{
		...
		if (bs->plantDecided > level.time || (bs->frame_Enemy_Len < BOT_PLANT_DISTANCE*2 && VectorLength(a) < BOT_PLANT_DISTANCE))
		{
			...
			mineSelect = BotSelectChoiceWeapon(bs, WP_GRENADE, 0);
	...
	if ( bs->dangerousObject && bs->dangerousObject->inuse && bs->dangerousObject->health > 0 &&
		...
		bs->cur_ps.weapon != WP_DET_PACK && bs->cur_ps.weapon != WP_GRENADE &&
		!bs->shootGoal )
	{
		float danLen;
		...
		AltFiring(bs))
	{
		...
		if (friendInLOF)
		{
			...
			{ //we have a friend here and are not currently using force powers, see if we can help them out
				...
				else if (friendInLOF->client->ps.fd.forcePower <= 50 && level.clients[bs->client].ps.fd.forcePower > forcePowerNeeded[level.clients[bs->client].ps.fd.forcePowerLevel[FP_LIFT]][FP_LIFT])
				{
					level.clients[bs->client].ps.fd.forcePowerSelected = FP_LIFT;
					useTheForce = 1;
					forceHostile = 0;
				}
			}
