...
qboolean FighterOverValidLandingSurface( Vehicle_t *pVeh )
{
	if ( pVeh->m_LandTrace.fraction < 1.0f //ground present
		&& pVeh->m_LandTrace.plane.normal[2] >= 0.5 )//flat enough
...
qboolean FighterIsLanding( Vehicle_t *pVeh, playerState_t *parentPS )
{
	...
	if ( FighterOverValidLandingSurface( pVeh )
		...
		&& parentPS->speed <= 0.5 )//going slow enough to start landing - was using pVeh->m_pVehicleInfo->speedIdle, but that's still too fast
	{
		return qtrue;
	}
	return qfalse;
...
//a predicted event or only doing it game-side. -rich
	...
	//speedMax /= 2;//[VehicleChanges] -- plan is to make vehicles slower when going forward and such, and smaller
