...
qboolean UnlockMemory( int address, int size )
{
		...
		DWORD dummy;
		//[OpenRP - Can't convert bool to qboolean]
		return (qboolean)( VirtualProtect( (LPVOID)address, size, PAGE_EXECUTE_READWRITE, &dummy ) != 0 );
		//[OpenRP - Can't convert bool to qboolean]
...
qboolean LockMemory( int address, int size )
{
		...
		DWORD dummy;
		//[OpenRP - Can't convert bool to qboolean]
		return (qboolean)( VirtualProtect( (LPVOID)address, size, PAGE_EXECUTE_READ, &dummy ) != 0 );
		//[/OpenRP - Can't convert bool to qboolean]
