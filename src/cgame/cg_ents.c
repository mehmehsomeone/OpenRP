...
//	int	col;
	 ...
	 if ( s1->weapon == WP_STUN_BATON )
   {
      ...
      void      *cgG2Self   = cg_entities[clientNum].ghoul2;
      vec3_t      rHandPos;
      mdxaBone_t   boltMatrix;

      trap_G2API_GetBoltMatrix( cgG2Self, 0, cgs.clientinfo[clientNum].bolt_rhand, &boltMatrix, cg_entities[clientNum].turAngles, cg_entities[clientNum].lerpOrigin, cg.time, cgs.gameModels, cg_entities[clientNum].modelScale );

      rHandPos[0] = boltMatrix.matrix[0][3];
      rHandPos[1] = boltMatrix.matrix[1][3];
      rHandPos[2] = boltMatrix.matrix[2][3];

      CG_TestLine( rHandPos, cent->currentState.pos.trBase, 1, 6, 1 );
      return;
   }
